...
const App: React.FC = () => {
  ...
  const handleStartShopping = () => {
    if (budget && budget > 0 && supermarket.trim() !== '') {
      ...
      setPurchaseFinalizedMessage(''); // Clear any previous finalized message
    } else {
  ...
  const recordPriceHistory = (itemName: string, itemPrice: number, market: string) => {
    const normalizedItemName = itemName.toLowerCase().trim();
  ...
  const updateItemInList = (itemId: string, newQuantity?: number, newPrice?: number) => {
    setShoppingList(prevList => prevList.map(item => {
      if (item.id === itemId) {
        ...
        // Re-fetch price insights if price changes, as it affects comparisons
  ...
  const updateItemInCart = (itemId: string, newQuantity?: number, newPrice?: number) => {
  ...
  const renderPriceInsight = (item: ShoppingItem) => {
    ...
    if (insights.length === 0) {
        insights.push(<p key="no-history" className="text-xs text-gray-500 italic">Sem histórico de preços para este item.</p>);
    }
    return insights;
  };
              ...
              <div className="mt-6">
                ...
                <RechartsAPI.ResponsiveContainer width="100%" height={200}>
                  <RechartsAPI.PieChart>
                    ...
                    <RechartsAPI.Tooltip formatter={(value: number, name: string) => [`R$${value.toFixed(2)}`, name]} />
                    <RechartsAPI.Legend />
                  </RechartsAPI.PieChart>
